...
PIN = 1 #defines which servo you want to move
PIN1= 2
PIN3 = 4
...
class ServoController1:
    def __init__(self, mav):
        self.mav = mav  # MAVLink connection instance
        self.servo1 = Servo(pin=PIN1)  # Initialize Servo with predefined pin

        # Write servo parameters function
    def write_servo_params1(self):
        print("Setting servo params...")
        for key, val in {  # call values defined above
            "MAX": self.servo1.max,
            "MIN": self.servo1.min,
            "TRIM": self.servo1.trim,
            "REVERSED": int(self.servo1.reversed)
        }.items():
            self.mav.mav.param_set_send(
                self.mav.target_system,
                self.mav.target_component,
                mav_bytes(f"SERVO{self.servo1.pin}_{key}"),  # Parameter name in byte format
                val,
                mavutil.mavlink.MAV_PARAM_TYPE_REAL32  # Parameter type
            )
        print("Done for servo in pin2 .")

    def send_angle_servo1(self, angle):
        pwm = self.servo1.angle_to_pwm(angle) #Convert angle to PWM
        print(f"Angle {angle}° → PWM {pwm}")
        self.mav.mav.command_long_send(
            self.mav.target_system,
            self.mav.target_component,
            mavutil.mavlink.MAV_CMD_DO_SET_SERVO, #Command to set servo position
            0, self.servo1.pin, pwm, 0, 0, 0, 0, 0
        )

class ServoController2:
    def __init__(self, mav):
        self.mav = mav  # MAVLink connection instance
        self.servo2 = Servo(pin=PIN3)  # Initialize Servo with predefined pin

            # Write servo parameters function

    def write_servo_params2(self):
        print("Setting servo params...")
        for key, val in {  # call values defined above
            "MAX": self.servo2.max,
            "MIN": self.servo2.min,
            "TRIM": self.servo2.trim,
            "REVERSED": int(self.servo2.reversed)
        }.items():
            self.mav.mav.param_set_send(
                self.mav.target_system,
                self.mav.target_component,
                mav_bytes(f"SERVO{self.servo2.pin}_{key}"),  # Parameter name in byte format
                val,
                mavutil.mavlink.MAV_PARAM_TYPE_REAL32  # Parameter type
            )
        print("Done for servo in pin2 .")
    #Send angle command to the servo function
    def send_angle_servo2(self, angle):
        pwm = self.servo2.angle_to_pwm(angle) #Convert angle to PWM
        print(f"Angle {angle}° → PWM {pwm}")
        self.mav.mav.command_long_send(
            self.mav.target_system,
            self.mav.target_component,
            mavutil.mavlink.MAV_CMD_DO_SET_SERVO, #Command to set servo position
            0, self.servo2.pin, pwm, 0, 0, 0, 0, 0
        )
...
if __name__ == "__main__":#allows this script to be ran independently
    ...
    controller1 = ServoController1(mav)
    controller1.write_servo_params1()
    controller2 = ServoController2(mav)
    controller2.write_servo_params2()
    controller.send_angle(0)
    controller1.send_angle_servo1(20)
    controller2.send_angle_servo2(30)
